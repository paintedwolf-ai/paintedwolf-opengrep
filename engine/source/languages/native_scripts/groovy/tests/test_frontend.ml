...
let () =
  let prefix = "// café\n" in
  let unsupported = "import example.Library" in
  ...
  Fun.protect ~finally:(fun () -> Sys.remove filename) (fun () ->
    ...
    let {Native_script_tree.parsed; diagnostics} = Parse_groovy_native.parse (Fpath.v filename) in
  ...
  let pattern = Parse_groovy_native.parse_pattern unsupported in
...
let () =
  let result = Parse_groovy_native.parse_pattern "if (true) { source()" in
...
let () =
  ...
  Fun.protect ~finally:(fun () -> Sys.remove filename) (fun () ->
    ...
    | [location] when location.Tok.pos.line = 3
        && location.pos.column = 0
        && location.pos.bytepos = String.length (prefix ^ source ^ "\n")
  ...
  let pattern = Parse_groovy_native.parse_pattern source in
  if not (List.exists (fun (error : Tree_sitter_run.Tree_sitter_error.t) ->
